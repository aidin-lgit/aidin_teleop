...
| 동작 | 키 / 페달 |
| --- | --- |
| 세션 시작 (1차) → 에피소드 녹화 + 제어 시작 (2차) | `a` (왼쪽 페달) |
| 에피소드 종료 → 홈 복귀, **세션 유지** | `a` (왼쪽 페달) |
| 세션 종료 | `c` (오른쪽 페달) |
| 긴급 정지 + 종료 | `b` (가운데 페달) |
...
2. **`a` 1차 입력** — 세션이 열리고 로봇이 초기 자세로 Homing.
3. **`a` 2차 입력** — 첫 에피소드 녹화 + 제어가 시작됨. 이 시점의 허리·머리 트래커 자세가 reference frame 으로 캡처됨.
4. **작업 수행** — 과일 1을 집어 상자에 넣고, 이어서 과일 2를 집어 상자에 넣은 뒤 손을 중립 위치로 복귀.
5. **`a` 입력** — 에피소드 녹화가 닫히고 로봇이 홈으로 복귀. **세션은 유지됨.**
6. **상자/과일 재배치** — 과일 위치를 변경하고, 다시 `a` 를 눌러 같은 세션에 다음 에피소드 녹화 시작 (4번으로 돌아가 반복).

모든 에피소드 수집이 끝나면 `c` 로 세션을 닫습니다. 전체 흐름과 상태 머신 상세는 [원격제어 → 원격제어 및 녹화 작업 프로세스](../../teleoperation/operation/#원격제어-및-녹화-작업-프로세스) 참고.

{{% notice style="warning" title="에피소드 사이에 `c` 를 누르지 마세요" %}}
`c` 는 **세션까지 닫습니다.** 에피소드 사이에 `c` 를 누르면 다음 `a` 가 새로운 `session_<timestamp>/` 폴더를 만들어 에피소드가 세션마다 흩어지고, 6절의 세션 일괄 변환에서 한 번에 묶이지 않습니다. 에피소드 반복은 `a`, 세션 종료만 `c` 입니다.
{{% /notice %}}

제어 중 로봇이 안전 정지(latch)로 멈추면 TELEOP 상태에서 `a` 가 `resume` 으로 동작해 **에피소드를 끊지 않고** 이어갈 수 있습니다 ([상세](../../teleoperation/operation/#제어기-입력-안전-검사-안전-정지)).
